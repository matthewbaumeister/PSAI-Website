...
core_fields AS (
  SELECT 
    ...
    CASE WHEN COUNT(*) FILTER (WHERE vendor_name IS NOT NULL) = COUNT(*) THEN '✅ 100%' ELSE '⚠️ ' || ROUND(100.0 * COUNT(*) FILTER (WHERE vendor_name IS NOT NULL) / NULLIF(COUNT(*), 0), 1) || '%' END as status,
    (SELECT vendor_name FROM dod_contract_news WHERE vendor_name IS NOT NULL LIMIT 1) as sample
  FROM dod_contract_news
  
  UNION ALL SELECT 'CORE FIELDS', 'vendor_state', COUNT(*) FILTER (WHERE vendor_state IS NOT NULL)::TEXT, CASE WHEN COUNT(*) FILTER (WHERE vendor_state IS NOT NULL) = COUNT(*) THEN '✅ 100%' ELSE '⚠️ ' || ROUND(100.0 * COUNT(*) FILTER (WHERE vendor_state IS NOT NULL) / NULLIF(COUNT(*), 0), 1) || '%' END, (SELECT vendor_state FROM dod_contract_news WHERE vendor_state IS NOT NULL LIMIT 1) FROM dod_contract_news
  UNION ALL SELECT 'CORE FIELDS', 'vendor_city', COUNT(*) FILTER (WHERE vendor_city IS NOT NULL)::TEXT, CASE WHEN COUNT(*) FILTER (WHERE vendor_city IS NOT NULL) = COUNT(*) THEN '✅ 100%' ELSE '⚠️ ' || ROUND(100.0 * COUNT(*) FILTER (WHERE vendor_city IS NOT NULL) / NULLIF(COUNT(*), 0), 1) || '%' END, (SELECT vendor_city FROM dod_contract_news WHERE vendor_city IS NOT NULL LIMIT 1) FROM dod_contract_news
  UNION ALL SELECT 'CORE FIELDS', 'award_amount', COUNT(*) FILTER (WHERE award_amount IS NOT NULL)::TEXT, CASE WHEN COUNT(*) FILTER (WHERE award_amount IS NOT NULL) = COUNT(*) THEN '✅ 100%' ELSE '⚠️ ' || ROUND(100.0 * COUNT(*) FILTER (WHERE award_amount IS NOT NULL) / NULLIF(COUNT(*), 0), 1) || '%' END, '$' || (SELECT ROUND(award_amount/1000000, 1) FROM dod_contract_news WHERE award_amount IS NOT NULL ORDER BY award_amount DESC LIMIT 1)::TEXT || 'M (largest)' FROM dod_contract_news
  UNION ALL SELECT 'CORE FIELDS', 'contract_number', COUNT(*) FILTER (WHERE contract_number IS NOT NULL)::TEXT, CASE WHEN COUNT(*) FILTER (WHERE contract_number IS NOT NULL) = COUNT(*) THEN '✅ 100%' ELSE '⚠️ ' || ROUND(100.0 * COUNT(*) FILTER (WHERE contract_number IS NOT NULL) / NULLIF(COUNT(*), 0), 1) || '%' END, (SELECT contract_number FROM dod_contract_news WHERE contract_number IS NOT NULL LIMIT 1) FROM dod_contract_news
  UNION ALL SELECT 'CORE FIELDS', 'service_branch', COUNT(*) FILTER (WHERE service_branch IS NOT NULL)::TEXT, CASE WHEN COUNT(*) FILTER (WHERE service_branch IS NOT NULL) = COUNT(*) THEN '✅ 100%' ELSE '⚠️ ' || ROUND(100.0 * COUNT(*) FILTER (WHERE service_branch IS NOT NULL) / NULLIF(COUNT(*), 0), 1) || '%' END, (SELECT STRING_AGG(DISTINCT service_branch, ', ') FROM dod_contract_news) FROM dod_contract_news
),
...
enhanced_fields AS (
  ...
  UNION ALL SELECT 'ENHANCED FIELDS', 'naics_code', COUNT(*) FILTER (WHERE naics_code IS NOT NULL)::TEXT, CASE WHEN COUNT(*) FILTER (WHERE naics_code IS NOT NULL) > 0 THEN '✅ ' || ROUND(100.0 * COUNT(*) FILTER (WHERE naics_code IS NOT NULL) / NULLIF(COUNT(*), 0), 1) || '%' ELSE '⚠️ None' END, (SELECT STRING_AGG(DISTINCT naics_code, ', ') FROM (SELECT DISTINCT naics_code FROM dod_contract_news WHERE naics_code IS NOT NULL LIMIT 3) t) FROM dod_contract_news
  UNION ALL SELECT 'ENHANCED FIELDS', 'solicitation_number', COUNT(*) FILTER (WHERE solicitation_number IS NOT NULL)::TEXT, CASE WHEN COUNT(*) FILTER (WHERE solicitation_number IS NOT NULL) > 0 THEN '✅ ' || ROUND(100.0 * COUNT(*) FILTER (WHERE solicitation_number IS NOT NULL) / NULLIF(COUNT(*), 0), 1) || '%' ELSE '⚠️ None' END, (SELECT solicitation_number FROM dod_contract_news WHERE solicitation_number IS NOT NULL LIMIT 1) FROM dod_contract_news
  ...
  UNION ALL SELECT 'ENHANCED FIELDS', 'contract_type', COUNT(*) FILTER (WHERE contract_type IS NOT NULL)::TEXT, CASE WHEN COUNT(*) FILTER (WHERE contract_type IS NOT NULL) > 0 THEN '✅ ' || ROUND(100.0 * COUNT(*) FILTER (WHERE contract_type IS NOT NULL) / NULLIF(COUNT(*), 0), 1) || '%' ELSE '⚠️ None' END, (SELECT STRING_AGG(DISTINCT contract_type, ', ') FROM (SELECT DISTINCT contract_type FROM dod_contract_news WHERE contract_type IS NOT NULL LIMIT 5) t) FROM dod_contract_news

-- ============================================
-- PROPSHOP.AI - COMPLETE DATABASE SCHEMA
-- ============================================
-- Run this entire script in Supabase SQL Editor
-- This creates ALL tables needed for the scraper
-- ============================================

-- Enable necessary extensions
CREATE EXTENSION IF NOT EXISTS "uuid-ossp";
CREATE EXTENSION IF NOT EXISTS "pg_trgm"; -- For text search
CREATE EXTENSION IF NOT EXISTS "vector"; -- For semantic search (optional, may fail if not available)
...
CREATE TABLE IF NOT EXISTS opportunity_master (
  ...
  opportunity_type TEXT, -- 'contract_award', 'solicitation', 'sbir', 'grant', 'other', 'modification'
  ...
  -- Timeline
  status TEXT, -- 'open', 'awarded', 'closed', 'forecasted', 'cancelled', 'modified'
...
CREATE TABLE IF NOT EXISTS user_opportunities (
  ...
  user_id UUID NOT NULL, -- From auth.users (will add later when auth is set up)
  opportunity_id BIGINT NOT NULL REFERENCES opportunity_master(id) ON DELETE CASCADE,
...
-- Text search indexes
CREATE INDEX IF NOT EXISTS idx_opp_master_title_trgm ON opportunity_master USING gin(title gin_trgm_ops);
CREATE INDEX IF NOT EXISTS idx_opp_master_description_trgm ON opportunity_master USING gin(full_description gin_trgm_ops);

-- Opportunity Sources indexes
...
CREATE INDEX IF NOT EXISTS idx_opp_sources_ingested_at ON opportunity_sources(ingested_at DESC);

-- User Opportunities indexes
...
-- Users can only see their own CRM opportunities (will work once auth is set up)
...
SELECT 
    ...
    ARRAY_AGG(DISTINCT os.source_name) FILTER (WHERE os.source_name IS NOT NULL) as source_names
FROM opportunity_master om
LEFT JOIN opportunity_sources os ON om.id = os.opportunity_id
GROUP BY om.id;

-- User's CRM pipeline view (will work once users exist)
...
-- Uncomment to insert a test opportunity
/*
INSERT INTO opportunity_master (
    ...
    'TEST:SAMPLE_OPP_001',
    'Test Contract Award - F-35 Maintenance',
    'Maintenance and sustainment services for F-35 aircraft',
    'This is a comprehensive maintenance contract for F-35 Lightning II aircraft including depot-level maintenance, component repair, and technical support services.',
    'award',
...
-- Run these after deployment to verify everything is set up correctly:

-- Check tables exist
-- SELECT table_name FROM information_schema.tables WHERE table_schema = 'public' AND table_name LIKE 'opportunity%' OR table_name LIKE 'chat%' OR table_name LIKE 'user_opportunities';

-- Check indexes exist
-- SELECT indexname FROM pg_indexes WHERE schemaname = 'public' AND tablename LIKE 'opportunity%';
...
-- Your database is now ready for the PropShop.ai scraper!
-- Next steps:
-- 1. Verify tables: SELECT COUNT(*) FROM opportunity_master;
-- 2. Run scraper: npm run scrape:propshop
-- 3. Check results: SELECT * FROM opportunity_master LIMIT 10;

COMMENT ON TABLE opportunity_master IS 'Master table for all opportunities from all sources - PropShop.ai';
